...
默认优先队列`priority_queue<int>`是对应的`priority_queue<int,vector<int>,less<int>>`的，不要搞混了
...
为了在算法竞赛中能简便地表示出图，我们通常用链式前向星来存储图。

链式前向星存储数据，读取遍历数据的复杂度都为$O(m)$,其中m为边数。

实现链式前向星是用的结构体，在结构体中存储当前点，指向点，长度。以此来表示图:
...
然后新写一个函数addedge来添加边与点,其中edge为那条边与所连的两个点，head建立两个相同当前点的联系，可以在遍历时决定遍历的深度
...
```c++
	for(int i = 1; i <= n; i++){
		for(int j = head[i]; j != -1; j = edge[j].next){
			cout << i << " " << edge[j].now << " " << edge[j].l << endl;
		}
	}
```

下面是完整代码展示（无向图，有向图不加第二个addedge就行）
...
拓扑排序用来解决那些只走一遍不重复但需要改变中间条件的问题（也可以不改变）

题目：

[HNOI2015\]菜肴制作_2021秋季算法入门班第九章习题：图论 (nowcoder.com)](https://ac.nowcoder.com/acm/contest/26077/1003)
...
bool tuopu(){
	pq.swap(null);
	for(int i = 1; i <= n; i++)
		if(inc[i] == 0) pq.push(i);
	int num = 0;
	while(!pq.empty()){
		int x = ans[++num] = pq.top();
		pq.pop();
		for(int i = head[x]; i != -1; i = edge[i].next){
			int y = edge[i].to;
			inc[y]--;
			if(inc[y] == 0) pq.push(y);
		}
	}
	if(num != n) return 0;
	else return 1;
...
一般是求最短路问题，在以下三种情况下不需要使用最短路思想就可以做出来：
...
特点：以起点为中心向外层层扩展，直到扩展到中心为止

适用于有向图和无向图，但不适用于有负权边的图

思路就是遍历一个点所有的入边，看加上后哪个最小，取最小的那个。同时，如果每个点都这么做，复杂度会达到$O(n^2)$为了降低复杂度,我们可以用优先队列，使用优先队列后，复杂度可以降为$O(n+m)log(m)$​,其中n为点数，m为边数。

这里用优先队列优化其实就是贪心的思想，每次拿离起点最近的那个点出来。



通过链式前向星建立无向图后，在无向图基础上进行遍历求解。

例题：

[1007-公交线路_2021秋季算法入门班第九章习题：图论 (nowcoder.com)](https://ac.nowcoder.com/acm/contest/26077/1007)
...
struct node2{
	int l;
	int x;
	bool operator < (const node2 &a)const //常成员函数，用const
	{
		return l > a.l;
	}
};
...
当遇到有负权的边时，迪杰斯特拉算法不再奏效，这时我们可以用贝尔曼-福特算法来解决。

贝尔曼福特算法可以解决有负权边的最短路的问题，但当遇到有权值和为负的环时，贝尔曼-福特算法会失效。

贝尔曼-福特算法的思路就是，依次遍历每一次由起始点发出的边，然后循环，直到每一个点都被遍历到并修改为最小值。

由于很直接很暴力，贝尔曼-福特算法的时间复杂度为$O(nm)$，想被卡住很容易。

此时我们可以用队列来优化，通过不断在队列中放入下一次需要用到的数，每次枚举的时候枚举与上次相比松弛的点，减少算法复杂度，这种优化被称作贝尔曼-福特的SPFA优化(Shortest Path Faster Algorithm)

这种写法与bfs很像，但与bfs不同的是，SPFA会不断的进出队列（因为有可能有的点需要不断被放入队列而bfs只需要放入一遍）在平均情况下，SPFA的时间复杂度为$O(km)$​,其中k为所有点的平均放入次数，m为边

在极端情况下，可以达成比未优化的贝尔曼-福特算法快n倍

同样，如果数据十分卡SPFA，那么SPFA会退化成原始的贝尔曼-福特的时间复杂度
...
当我们需要求出多个原点的最短路时，可以用弗洛伊德算法(Floyd)来解决

###### 弗洛伊德算法(Flyod)

弗洛伊德算法是通过动态规划思想来对图上每一点进行计算的算法，也因如此，可以求多原点问题。
...
最小生成树算法主要是解决多边建树需求最小权值问题，典型的方法有Prim算法与Kruskal算法

##### 普利姆算法(Prim)

Prim算法的思想主要是从原点开始，每次向最小且没有连过的点连一条边，然后每次都从第一个点再次出发查找连接一次。

比如，给定n个点m条边，然后给出每个边起始点和终止点以及权值，求最小生成树：

需要用到优先队列去每次取最小的边

[1008-挖沟_2021秋季算法入门班第九章习题：图论 (nowcoder.com)](https://ac.nowcoder.com/acm/contest/26077/1008)
...
Kruskal算法与Prim的不同之处在于连边方式：采用并查集进行连边的检查操作，以避免同一条边被连两次。

通过sort进行排序后，可以把需要的由短到长的边按顺序取出来使用。
...
void solve(){
	...
	for(int i = 1; i <= m; i++){//克鲁斯卡尔核心算法，利用并查集查重边，防止重复
		int fto = find(edge[i].to);
		int fnext = find(edge[i].next);
		if(fto == fnext) continue;
		ans += edge[i].len;
		fa[fto] = fnext;
	}
	cout << ans << endl;
...
在核心算法中，可以加上一个cnt记录已经添加的边，然后当大于n-1条边时就可以退出

![image-20240410085625515](https://cdn.jsdelivr.net/gh/Zeratuli/Image@main/img/202404100856970.png)

题型

![image-20240411142837598](https://cdn.jsdelivr.net/gh/Zeratuli/Image@main/img/202404111428973.png)

方法有两种：
1. 转换思路，让终点变为起点，变成单原点求最短路

2. 在所有k个点前加一个点，使得这个点到k个点，到每一个点的距离为0（len = 0）（超级原点），然后求单原点最短路

- 当终点也是多个时找最短的，也可以用2的方法，再加一个超级终点就行

### 数学

![image-20240411144933005](C:\Users\54219\AppData\Roaming\Typora\typora-user-images\image-20240411144933005.png)
